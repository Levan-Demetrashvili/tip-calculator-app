    ...
    <div className='tip-container'>
      ...
      <div className='rates-container'>
        {percentages.map(perc => (
          <RateBox key={uuid()} percent={perc} tipRate={tipRate} setTipRate={setTipRate} />
        ))}
...
function RateBox({ percent, tipRate, setTipRate }) {
  return (
    <label className='rate' htmlFor={`${percent}%`}>
      <input id={`${percent}%`} type='radio' name='rate' checked={percent === tipRate ? true : false} onChange={() => setTipRate(percent)} />
      <span>{percent}%</span>
    </label>
  );
}
